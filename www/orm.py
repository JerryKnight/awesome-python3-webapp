...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        logging.info('找到模块: %s (table: %s)' % (name, tableName))
        mappings = dict()
        fields = []
        primaryKey = None
        for k, v in attrs.items():
            if isinstance(v, Field):
                logging.info('找到映射关系: %s ===> %s' % (k, v))
                mappings[k] = v
                if v.primary_key:
                    #找到了主键
                    if primaryKey:
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def __getattr__(self, key):
        try:
            ...
            raise AttributeError(r"'模型' 没有这个属性 '%s'" % key)
    ...
    async def save(self):
        ...
        if rows != 1:
            logging.warning('保存记录失败，影响记录数：%s' % rows)
    ...
    async def update(self):
        ...
        if rows != 1:
            logging.warning('修改记录失败，影响记录数：%s' % rows)

    async def delete(self):
        args = [self.getValue(self.__primaryKey__)]
        rows = await execute(self.__delete__, args)
        if rows != 1:
            logging.warning('删除记录失败，影响记录数：%s' % rows)